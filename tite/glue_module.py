...
# The following hyperparameters are taken from the paper
# https://arxiv.org/pdf/2312.17482
# Task learning_rate beta epsilon weight_decay epochs
# MNLI 5e-5 [0.9, 0.98] 1e-6 5e-6 3
# QNLI 1e-5 [0.9, 0.98] 1e-6 1e-6 10
# QQP 3e-5,1.5e-5 [0.9,0.988] 1e-6 3e-6 5
...
# STS 3e-5 [0.9, 0.98] 1e-6 3e-6 10
BETAS_MAP = {
    "cola": (0.9, 0.98),
    "sst2": (0.9, 0.988),
    "mrpc": (0.9, 0.98),
...
class GlueModule(LightningModule):
    ...
    def validation_step(self, batch, *args, **kwargs) -> None:
        ...
        for validation_metric in self.evaluation_metrics:
            metric = validation_metric(logits.view(-1), batch["label"].view(-1))
    ...
    def configure_optimizers(self) -> torch.optim.Optimizer:
        opt = torch.optim.AdamW(self.parameters(), lr=LEARNING_RATE_MAP[self.task], weight_decay=WEIGHT_DECAY_MAP[self.task], betas=BETAS_MAP[self.task], eps=1e-6)
        return opt
